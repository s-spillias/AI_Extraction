...
ggsave("Figures/overall.png", width = 7, height = 4, device = "png")
```
...
overall <- ggplot(stat_summary, aes(x = AI, y = mean, fill = Criteria, group = Criteria)) +
  ...
  coord_cartesian(ylim = c(-1, 1)) +
  facet_wrap(rev(group_agent)~Criteria) +
  #scale_y_continuous(    labels = c("Poor","Fair","Good"),breaks = c(1,2,3),) +
  scale_fill_manual(values = crit_pal) +
  theme(legend.position = 'bottom') 
plot(overall)

ggsave("CBFM/Manuscript/Figures/overall-withAI.png", width = 7, height = 5, device = "png")
```
