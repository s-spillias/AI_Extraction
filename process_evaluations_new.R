...
### run other process evaluation FIRST to get proper objects/dataframes to run in this code
...
ai_extractions <- sheets %>% lapply(function(x) read_excel(file_path, sheet = x) %>% as_tibble() %>% 
                                      ...
                                      mutate(across(-Citation, nchar))
                                    ) %>% 
  setNames(sheets)

human_extraction = "../Extraction_Output/Human_Extraction_Cleaned.xlsx" %>% 
...
verbosity_ratios = sheets %>% lapply(function(x) bind_cols(human_extraction$Question,
  ( ai_extractions[[x]] %>% select(-1) ) /
    ( human_extraction %>% select(-1) )
  ))
